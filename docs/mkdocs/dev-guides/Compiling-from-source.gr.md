...
!!! σημείωση
    Σε όλα τα βήματα υποθέτω ότι είσαι στον φάκελο `/{{app_name}}`.  Όλες οι εντολές υποθέτουν ότι θα βρουν το `{{app_name}}.csproj` στον φάκελο που τρέχουμε τις εντολές.
...
Είναι ανάλογο με το από πάνω αλλά χωρίς παραμέτρους `./{{app_name}}.exe`. Οπότε τρέχουμε αυτό αν θέλουμε να βάλουμε παραμέτρους:
```powershell
...
Ένα καλό μέρος για να αρχίσουμε το έργο είναι το [CliCommands folder](https://github.com/tpill90/{{repo_name}}/tree/master/{{app_name}}/CliCommands).  Αυτός ο φάκελος περιέχει όλες τις εντολές που μπορούμε να τρέξουμε, όπως `prefill` ή `select-apps`.  
