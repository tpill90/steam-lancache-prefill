...
Prior to installing {{app_name}} via docker you should ensure you have a working Lancache caching and Lancache DNS server installed on your system.
...
## Installing {{app_name}} Docker Image

Again from your Unraid terminal you will download and run the **{{app_name}}** Docker image.

!!! note
    ```--add-host={{cache_trigger_domain}}:XXX.XXX.XXX.XXX``` is required in order to tell the container the IP Address of your Lancache server running on your custom network.  If not specified, **{{app_name}}** will be unable to locate the Lancache server, and will be unable to prefill.
...
docker run -it --rm \
  ...
  --volume /mnt/user/appdata/{{app_name}}:/Config \
  tpill90/{{repo_name}}:latest \
  select-apps
```

To get familiarized with how to use **{{app_name}}**, see the [Getting Started](https://github.com/tpill90/{{repo_name}}#getting-started) guide.
...
Next, add a new script by clicking *ADD NEW SCRIPT*, and give it the name `{{app_name}}`.  After it has been created, click the orange gear next to `{{app_name}}`, and select *EDIT SCRIPT*.
...
```bash
#!/bin/bash
docker rm -f {{app_name}} &>/dev/null && echo 'Removed old container from previous run';
...
Finally after entering a cron expression, click *APPLY* to save your cron expression.  You should now be all set to have **{{app_name}}** run on a schedule!
