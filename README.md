...
| --quiet     |     |             | Reduces noisy log output.  By default, games that are already up to date will show a message that they are already up to date.  Specifying this option will make it so only games that have updates will be logged.  |
