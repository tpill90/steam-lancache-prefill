...
namespace SteamPrefill.CliCommands
{
    ...
    {
        public override IPAddress Convert(string overrideIp)
        {
            if (overrideIp == null)
            {
            ...
            if (!IPAddress.TryParse(overrideIp, out var ipAddress))
            {
                AnsiConsole.MarkupLine(Red($"{White(overrideIp)} is not a valid IP address!"));
