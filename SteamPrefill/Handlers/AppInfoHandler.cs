namespace SteamPrefill.Handlers
{
    ...
    public class AppInfoHandler
    {
        ...
        public async Task<List<AppInfo>> GetAvailableGamesByIdAsync(List<uint> appIds)
        {
            ...
            var asd = appInfos.Where(e => e.ExcludeFromFamilySharing).ToList();
            ...
            var filteredGames = appInfos.Where(e => (e.Type == AppType.Game || e.Type == AppType.Beta || e.Type == AppType.Demo)
                                                    ...
                                                    && e.SupportsWindows
                                                    // TODO need to be able to look up who owns it, and only exclude it if it came from another account.
