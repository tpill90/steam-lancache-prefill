...
DOWNLOAD_URL="https://github.com/tpill90/steam-lancache-prefill/releases/download/v${LATEST_TAG}/SteamPrefill-${LATEST_TAG}-linux-${ARCH}.zip"
wget -q -nc --show-progress --progress=bar:force:noscroll $DOWNLOAD_URL

# Unzip
echo -e "${Yellow} Unzipping... ${NC}"
unzip -q -j -o SteamPrefill-${LATEST_TAG}-linux-${ARCH}.zip

# Required so executable permissions don't get overwritten by unzip
chmod +x SteamPrefill update.sh

# Cleanup
rm SteamPrefill-${LATEST_TAG}-linux-${ARCH}.zip
